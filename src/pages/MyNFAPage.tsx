...
export function MyNFAPage({ account, ownedTokens, isScanning }: MyNFAPageProps) {
    ...
    const networkLabel = CHAIN_CONFIG.rpcUrl.includes('base') ? 'BASE MAINNET' : 'CUSTOM RPC';
...
node --loader ts-node/esm scripts/agent-runner.ts`;
    ...
    const handleExecuteAction = async () => {
        ...
        try {
            ...
            await tx.wait();
            alert("Action Executed! Check Basescan for 'ActionExecuted' event.");
        } catch (err: any) {
            console.error(err);
    ...
    const downloadAgentJson = (id: number) => {
        const data = {
            ...
            personality: {
                bio: "A digital entity living in AI Village.",
                traits: ["Unknown"],
                voice: "default"
            }
        };
                                ...
                                {runtimePanelOpen && (
                                    <div style={{ padding: '12px', display: 'grid', gap: '10px' }}>
                                        ...
                                        <div className="ga-help-text">
                                            ENV DEFAULTS: `VITE_BASE_RPC_URL`, `VITE_TOKEN_ADDRESS` | Runtime: `LEGACY_MODE`
                                        </div>
                                    </div>
                                )}
                            </div>
